...
func NewPostgresSchemaString(t *testing.T, sql string, opts ...Option) (*pgxpool.Pool, CleanupFunc) {
	t.Helper()
	// Create a new schema.
	...
	schemaConn, err := pgxpool.NewWithConfig(ctx, connCfg)
	if err != nil {
...
func NewPostgresSchema(t *testing.T, sqlFiles []string, opts ...Option) (*pgxpool.Pool, CleanupFunc) {
	t.Helper()
	sb := &strings.Builder{}
